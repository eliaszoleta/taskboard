...
async function handleCreateSpace() {
  ...
  if (!raw) { errEl.textContent = 'Please enter a name for your taskboard.'; return; }
...
async function handleOpenSpace() {
  ...
  if (!raw) { errEl.textContent = 'Please enter your taskboard name.'; return; }
  const key = toSpaceKey(raw);
  if (!key) { errEl.textContent = 'Please use letters, numbers, or spaces.'; return; }
  ...
  try {
    const snap = await get(ref(db, `spaces/${key}/meta`));
    if (!snap.exists()) {
      errEl.textContent = 'No taskboard found with that name. Check the spelling.';
      btn.disabled = false;
